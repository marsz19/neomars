...
function title() {
	  console.log(chalk.bold.blue(figlet.textSync('MarsZxY BotsZ', {
		font: 'Standard',
	...
	console.log(chalk.yellow(`\n ${chalk.green('[ Created By MarselStore ]')}\n`))
}
/**
...
const connectToWhatsApp = async () => {
	const conn = makeWASocket({
            ...
            auth: state,
            browser: ["MarsZxY BotsZ Multi Device", "Safari", "3.0"]
        })
	title()
        store.bind(conn.ev)
	...
	conn.ev.on('messages.upsert', async m => {
		...
		require('./message/msg')(conn, msg, m, setting, store, welcome, set_proses, set_done)
	})
